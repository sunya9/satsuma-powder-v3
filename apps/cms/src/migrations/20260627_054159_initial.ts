...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`_posts_v\` (
  	...
  	FOREIGN KEY (\`version_feature_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`version_meta_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`_posts_v_parent_idx\` ON \`_posts_v\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_posts_v_version_version_slug_idx\` ON \`_posts_v\` (\`version_slug\`);`)
  await db.run(sql`CREATE INDEX \`_posts_v_version_version_feature_image_idx\` ON \`_posts_v\` (\`version_feature_image_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`tags_feature_image_idx\` ON \`tags\` (\`feature_image_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`authors_profile_image_idx\` ON \`authors\` (\`profile_image_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`site_settings_icon_idx\` ON \`site_settings\` (\`icon_id\`);`)
