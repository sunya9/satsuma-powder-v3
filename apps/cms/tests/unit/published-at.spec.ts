...
describe('resolvePublishedAt', () => {
  it('returns now when published and publishedAt is unset', () => {
    expect(resolvePublishedAt({ isPublished: true, currentPublishedAt: undefined, now: NOW })).toBe(
      NOW,
    )
  ...
  it('returns undefined (keep existing) when published and already set', () => {
    expect(
      resolvePublishedAt({
        isPublished: true,
        currentPublishedAt: '2025-01-01T00:00:00.000Z',
        now: NOW,
      }),
    ).toBeUndefined()
  })
  ...
  it('returns undefined (untouched) when not published (draft / unpublish)', () => {
    expect(
      resolvePublishedAt({ isPublished: false, currentPublishedAt: undefined, now: NOW }),
    ).toBeUndefined()
    expect(
      resolvePublishedAt({
        isPublished: false,
